...
NodoC* ListaConductores::buscar(int cod) {
    NodoC *p = lista;
    
    while (p) {
        if (p->conductor.GetDni() == cod) break;
        p = p->der;
    }
...
void ListaConductores::leerInfraccion(ifstream &arch) {
    int cod;
    NodoC *c;
    ...
    c = buscar(cod);
    if (c) {
        c->conductor.leerInfraccion(arch);
    }
}

void ListaConductores::acumularMultas(int dni, char *nom, int &tel, int *inf) {
    NodoC *c;
    
    c = buscar(dni);
    c->conductor.GetNombre(nom);
    tel = c->conductor.GetTelefono();
    
    c->conductor.acumularMultas(inf);    
}
